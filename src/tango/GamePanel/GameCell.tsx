...
const GridCell = ({ val, rightRelation, bottomRelation, onClick, canChange, isValid }: { val: number, rightRelation: number, bottomRelation: number, onClick: MouseEventHandler, canChange: boolean, isValid: boolean }) => {
    const bgColor = (canChange ? 'bg-white' : 'bg-black/20') +  (!isValid ? ' bg-[repeating-linear-gradient(45deg,_rgba(255,0,0,0.3)_0,_rgba(255,0,0,0.3)_2px,_transparent_2px,_transparent_8px)]' : '');

    return (
        <button
            className={`relative border border-black/20 aspect-square p-4 ${bgColor} ${canChange ? 'cursor-pointer' : 'cursor-not-allowed'}`}
            onClick={onClick}
        >
            {val === CELL_TYPE.TYPE_A ? <img src={TypeASVG} alt="Sun" /> : <></>}
            {val === CELL_TYPE.TYPE_B ? <img src={TypeBSVG} alt="Moon" /> : <></>}
